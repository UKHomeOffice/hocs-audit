    ...
    public void export(OutputStream outputStream, boolean convertHeader) throws IOException {
        ...
        try (var buffer = new BufferedOutputStream(outputStream);
             ...
             var printer = new CSVPrinter(outputWriter, CSVFormat.Builder.create()
                     ...
                     .setAutoFlush(true)
                     .build())) {
            var topics = infoClient.getTopics();

            topics.forEach(topic -> {
                try {
                    printer.printRecord(topic.getValue(), topic.getLabel(), topic.isActive());
                } catch (IOException e) {
                    throw new ApplicationExceptions.AuditExportException(e, EXPORT_FAILURE_TOPIC_ROW, "Unable to export topic row for {}", topic.getLabel());
                }
            });
        }
