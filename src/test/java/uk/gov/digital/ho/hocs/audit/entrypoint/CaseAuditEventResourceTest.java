...
public class CaseAuditEventResourceTest extends BaseExportResourceTest {
    ...
    @BeforeEach
    public void setup() {
        auditEvent = new AuditEvent(UUID.randomUUID(), UUID.randomUUID(), "TEST", "TEST", "{}", "TEST",
    ...
    public void shouldDeleteCaseAudit() {
        ...
        ResponseEntity<DeleteCaseAuditResponse> result = restTemplate.exchange(
            getExportUri("/audit/case/%s/delete", auditEvent.getCaseUUID()), POST, httpEntity,
            DeleteCaseAuditResponse.class);
